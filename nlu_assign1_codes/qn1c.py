...
train_uni3= train_uni + train_uni1
#print(len(train_uni3))
test_uni3=test_uni + test_uni1
#print(len(test_uni3))
size3= size + size1
#print(len(size3))
tok3=tok + tok1
...
add=0
add1=0
for x in range(0,train_len3,1):
    train_uni_1st3.append(tok3[add])    ##for uni_first letters
    add=add+size3[x]
    
    
print(len(train_uni_1st3))   ##for uni_first letters
print(add)
#for uni_first letters  counts of it in the dictionary
for i in range(0,len(train_uni_1st3),1):
    if train_uni_1st3[i] in train_uni_1st_dict3:
        train_uni_1st_dict3[train_uni_1st3[i]] +=1
    else:
        train_uni_1st_dict3[train_uni_1st3[i]]=1
...
for y in range(train_len3,train_len3+test_len3,1):
    add1=add1+size3[y]
    
print(add1)

for k in range(0,add):
    if tok3[k] in train_uni_dict3:
        train_uni_dict3[tok3[k]] +=1
    else:
        train_uni_dict3[tok3[k]]=1
    train_uni3.append(tok3[k])        ########D1-TRAIN SAMPLE####
    
print(len(train_uni3))

for k in range(add,add1+add):
    if tok3[k] in test_uni_dict3:
        test_uni_dict3[tok3[k]] +=1
    else:
        test_uni_dict3[tok3[k]]=1
    test_uni3.append(tok3[k])       ########D1-TEST SAMPLE####
    
print(len(test_uni3))

for z in range(add1+add,len(tok3),1):
    if tok3[k] in dev_uni_dict3:
        dev_uni_dict3[tok3[k]] +=1
    else:
        dev_uni_dict3[tok3[k]]=1
    dev_uni3.append(tok3[k])
    
print(len(dev_uni3))####DEV SAMPLE####
L=len(size3)
...
train_quad_dict3={}
train_quad_1st3=[]
train_quad_1st_dict3={}
j=0
for x in range(0,train_len3,1):
    train3=[]  ## only for storing a particular sentence or line
    r=size3[x]
    #print(r)
    i=0
    while i<r:
        train3.append(tok3[j+i])
        i=i+1
    ...
    #print(j)
    train_b3=[]
    train_b3=bigram(train3,r)
    #print(train_b)
    
    for k in range(0,len(train_b3),1):
        if train_b3[k] in train_bi_dict3:
            train_bi_dict3[train_b3[k]] +=1
        else:
            train_bi_dict3[train_b3[k]]=1
        train_bi3.append(train_b3[k])
        if k==0:
            train_bi_1st3.append(train_b3[k])        ###for bi_first letters list and its occurence in sample
    ...
    train_t3=[]
    train_t3=trigram(train3,r)
    #print(train_b)
    
    for l in range(0,len(train_t3),1):
        if train_t3[l] in train_tri_dict3:
            train_tri_dict3[train_t3[l]] +=1
        else:
            train_tri_dict3[train_t3[l]]=1
        train_tri3.append(train_t3[l])
        if l==0:
            train_tri_1st3.append(train_t3[l])  ###for tri_first letters list and its occurence in sample
        
        
    
    
    train_q3=[]
    train_q3=quadgram(train3,r)
    #print(train_b)
    
    for o in range(0,len(train_q3),1):
        if train_q3[o] in train_quad_dict3:
            train_quad_dict3[train_q3[o]] +=1
        else:
            train_quad_dict3[train_q3[o]]=1
        train_quad3.append(train_q3[o])
        if o==0:
            train_quad_1st3.append(train_q3[o])  ###for quad_first letters list and its occurence in sample
        
        

    
for i in range(0,len(train_bi_1st3),1):##for bi_first letters  counts of it in the dictionary
    if train_bi_1st3[i] in train_bi_1st_dict3:
        train_bi_1st_dict3[train_bi_1st3[i]] +=1
    else:
        train_bi_1st_dict3[train_bi_1st3[i]]=1   
#for tri_first letters  counts of it in the dictionary
for i in range(0,len(train_tri_1st3),1):
    if train_tri_1st3[i] in train_tri_1st_dict3:
        train_tri_1st_dict3[train_tri_1st3[i]] +=1
    else:
        train_tri_1st_dict3[train_tri_1st3[i]]=1
        
#for quad_first letters  counts of it in the dictionary
for i in range(0,len(train_quad_1st3),1):
    if train_quad_1st3[i] in train_quad_1st_dict3:
        train_quad_1st_dict3[train_quad_1st3[i]] +=1
    else:
        train_quad_1st_dict3[train_quad_1st3[i]]=1
        
    
'''print(len(train_bi3))
print(len(train_bi_dict3))

print(len(train_tri3))
print(len(train_tri_dict3))

print(len(train_quad3))
print(len(train_quad_dict3))
##print(len(train_uni_1st))
print(len(train_bi_1st3))
print(len(train_tri_1st3))
print(len(train_quad_1st3))
print(len(train_bi_1st_dict3))
print(len(train_tri_1st_dict3))
print(len(train_quad_1st_dict3))'''

##dont change anything over here in dataset ,train_class etc
import math
import numpy as np
...
##print(L)
train_len3=int(L*0.8)
test_len3=int(L*0.1) ### can be used for calculating the no of sentences in test data
##print(test_len)
...
for x in range(train_len3,train_len3+test_len3,1):
    test3=[]  ## only for storing a particular sentence or line
    r=size3[x]
    #print(r)
    i=0
    while i<r:
        
        test3.append(test_uni3[j+i])
        i=i+1
    ##train_uni_1st.append(train[0])##for uni_first letters
    ...
    test_b3=[]
    test_b3=bigram(test3,r)
    #print(train_b)
    
    for k in range(0,len(test_b3),1):## DONT change train and test in this line
        if test_b3[k] in test_bi_dict3:
            test_bi_dict3[test_b3[k]] +=1
        else:
            test_bi_dict3[test_b3[k]]=1
        test_bi3.append(test_b3[k])

        
    ###forming test trigrams
    test_t3=[]
    test_t3=trigram(test3,r)
    #print(train_b)
    
    
    for k in range(0,len(test_t3),1):## DONT change train and test in this line
        if test_t3[k] in test_tri_dict3:
            test_tri_dict3[test_t3[k]] +=1
        else:
            test_tri_dict3[test_t3[k]]=1
        test_tri3.append(test_t3[k])
    ...
    #for x in range(0,len(test)):
        ...
        #else:
            #c=c+1
    if len(test3)!=0:  ###as the lent(test) might go to zero
        #if c!=0:
            #print('out')
        per_uni3=add_1_probability(test3,train_uni_dict3,len(train_uni3),len(train_uni_dict3),len(test3))
        #else:
            #print('hello')
           # per_uni=probability(test,train_uni_dict,len(train_uni),len(train_uni_dict),len(test))###helps in better perplexity
        per_un3=per_un3 + per_uni3
    
    ## for BI-GRAMS OF TESTING AND ITS PERPLEXITY CALCULATION 
    #print(len(test_b))
    if len(test3)!=0:###as the lent(test) might go to zero
        per_bi3=bigram_add_1(test3,train_uni_dict3,train_uni_1st_dict3,len(train_uni_dict3),len(test3))
        per_b3=per_b3 + per_bi3
    
    ## for TRI-GRAMS OF TESTING AND ITS PERPLEXITY CALCULATION
    if len(test3)!=0:###as the lent(test) might go to zero
        
        per_tri3=trigram_add_1(test3,train_uni_dict3,train_bi_dict3,test_b3,train_uni_1st_dict3,len(train_uni_dict3),len(test3))
        per_t3=per_t3 + per_tri3


#print(len(train_uni_1st)) ###BOTH THE LENGTHS ARE SAME
#print(train_len)
print("PERPLEXITY of unigram=",per_un3/len(train_uni3)) ###train_len==len(train_uni_1st) sice,total no of
print("PERPLEXITY of bigram=",per_b3/len(train_bi_dict3)) ###train_len==len(train_uni_1st) sice,total no of
print("PERPLEXITY of trigram=",per_t3/len(train_tri3)) ###train_len==len(train_uni_1st) sice,total no of
